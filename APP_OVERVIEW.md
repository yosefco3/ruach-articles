...
- **`iching`** — `getContent` (ציבורי: מחזיר hexagrams+trigrams+intro ממוזגים);
  `refineQuestion` (ציבורי, **לפני ההטלה**: בודק את ניסוח השאלה ומציע עד 2 ניסוחים חלופיים
  כשהיא בעייתית בבירור; חופשי לכולם, מוגבל-קצב פר-IP (`ICHING_REFINE_RATE_PER_HOUR`),
  fail-open מלא — חריגת קצב/שגיאה → `{problematic:false}`; נשלט בדגל `intro.refineEnabled`);
...
BlogPosting+BreadcrumbList במאמר, FAQPage ב-`/derech` — נבנית דינמית מתוכן הדף
החי ב-DB עם fallback ל-default, `resolveDerechSeo`). **נקודות-קצה לסורקים:**
`server/sitemap.ts` (`/sitemap.xml`), `server/robots.ts` (`/robots.txt`),
`server/rss.ts` (`/rss.xml`, מפורסם ב-head כ-`rel="alternate"`), `server/llmstxt.ts`
(`/llms.txt` — אינדקס markdown לסוכני AI לפי llmstxt.org, נבנה מה-DB) — כולם
נרשמים ב-`_core/startup/seo-routes.ts`. הנחיות כתיבה מוכוונות-GEO (מבנה
תשובה-קודם, ציטוטים עם ייחוס): `docs/GEO-WRITING.md`.
...
|---|---|---|
| 2026-09-10 | **AI provider → DeepSeek V4-Pro (reasoning model)**: `DEEPSEEK_MODEL` default `deepseek-chat`→`deepseek-v4-pro` (API id undated; the release is V4-Pro-0813). The model emits `reasoning_content` before `content` and reasoning counts inside `max_tokens`, so the provider layer adds `REASONING_HEADROOM` (4000, deliberately generous — thinking is wanted, model is cheap) on top of every caller's answer budget, and reports a distinct error when the budget dies mid-reasoning (empty content + non-empty reasoning). Serves both tarot and I Ching. No Railway env override exists, so the code default takes effect on deploy. +2 tests. | `server/_core/{env,aiProvider}.ts`, `server/ichingAi.provider.test.ts` |
| 2026-09-10 | **Tarot AI: center-card reading method** — `buildTarotPrompt` rewritten: card 2 (displayed center) carries the answer; side cards read either as two complementary facets or as supporter+opposer (model picks per the actual cards, with a stated rationale and a contradiction decision rule); adds an internal work-order (understand the question → distill the center → relate sides → self-check) and a matching answer structure. Replaces the 2026-09-09 free-frame decision. Tests updated (421 stable). | `server/tarotAi.ts`(+test), `APP_OVERVIEW.md` |
| 2026-09-10 | **Gallery lightbox + English deck ZIP + CDN cache-busting**: gallery tiles open an enlarged lightbox (Esc/click closes); a second free download `/tarot-cards/ruach-tarot-deck-en.zip` serves English-labeled cards from the new `client/public/tarot-cards/en/` (79 webp; `label.py --en` + `export_site_assets.py --en` in the tarot-deck pipeline); `tarotDeckZip.ts` parameterized per variant (separate caches). `DECK_ASSETS_VERSION` (=2) appended as `?v=` to all card/back/zip URLs — bump it after any image replacement to bust the Cloudflare/browser cache (old assets were cached by filename). +3 tests (419→421, 1 skipped). | `server/tarotDeckZip.ts`(+test), `server/_core/startup/seo-routes.ts`, `shared/tarot/cards.ts`(+test), `client/src/pages/{TarotDeckGallery,TarotReading}.tsx`, `client/public/tarot-cards/en/` |
| 2026-09-10 | **Tarot deck finalized + gallery page** (`feature-prompts/tarot-gallery/`): (1) All 78 card images finished and QA'd — traditional names restored everywhere (הכהן הגדול/המוות/השטן/יום הדין kept; התלוי→האיש התלוי, המיזוג→האיזון per the canonical label list), art brought back close to Rider-Waite compositions (horned Devil holding the couple's rope, trumpet herald in Judgement, pierced heart in 3 of Swords, hand-from-cloud aces, Star woman pouring both jugs into the pool) with only crosses and nudity excluded; every minor pip visually verified to show the EXACT emblem count (3 generation rounds via local ComfyUI + targeted PIL retouches — count-critical scenes phrased as explicit row arrangements); court coins/cups enlarged; new ornamental `back.webp`. Assets: 79 × ~600px webp committed at `client/public/tarot-cards/` (the deck-ZIP endpoint now serves for real). (2) New public gallery `/tarot/deck` (`TarotDeckGallery.tsx`, pure `deckSections()` helper in `pages/tarot/model.ts`): 5 sections (major + 4 suits with elements), RTL grid, lazy images, credit line (Claude Fable 5 + Z-Image Turbo), links to/from `/tarot`; SEO head (`TAROT_DECK_SEO`), sitemap (0.7) + llms.txt entries. +4 tests (415→419). | `client/public/tarot-cards/` (79 webp), `shared/tarot/cards.ts`(+test), `client/src/pages/TarotDeckGallery.tsx`, `client/src/pages/tarot/model.ts`(+test), `client/src/pages/TarotReading.tsx`, `client/src/routes/{lazy.ts,public.tsx}`, `server/{seo,sitemap,llmstxt}.ts`(+tests) |
...
| 2026-08-08 | **Derech page admin editor** (`feature-prompts/derech-page/` phase 2): the /derech method page is now editable at `/admin/derech` (linked from the admin quick-links). Content lives as structured JSON (`DerechContent` in `shared/derech.ts`) in the new `derechPage` table (`content` mediumtext; migration `0016_derech_page.sql`, idempotent `0016-derech-prod.sql` for Railway — **must be run on prod before saving from the editor works**). New `derech` tRPC router: `get` public (null ⇒ client renders `DEFAULT_DERECH_CONTENT`, so the page survives a missing table/row), `update` admin-only with full zod shape validation. `Derech.tsx` is content-driven (layout fixed in code; principle icons assigned by index); text fields support markdown-lite — `**bold**` + `[label](href)` — rendered by `renderMdLite` (`client/src/lib/mdLite.tsx`, JSX-safe, no HTML). The admin form edits every section: hero titles, opening paragraphs (blank-line separated), principles (add/remove/reorder), in/out lists (line per item), closing. `/derech` added to ssrData prefetch. +12 tests (242→254 in vitest terms; suite now 311). | `drizzle/schema.ts`+`0016_*.sql`, `shared/derech.ts`, `server/db/derech.ts`, `server/db.ts`, `server/routers/derech.router.ts`(+test), `server/routers/index.ts`, `client/src/lib/mdLite.tsx`(+test), `client/src/pages/Derech.tsx`, `client/src/pages/AdminDerech.tsx`, `client/src/pages/AdminPage.tsx`, `client/src/routes/{lazy.ts,admin.tsx,ssrData.ts}` |
| 2026-08-08 | **"דרך הרוח" method page** (`feature-prompts/derech-page/`): dedicated static page at `/derech` presenting the site's method — "דרך הרוח — הנבואה הטבעית" (source of truth: `METHOD.md`, uncommitted content doc): the name, the five selection principles, an in/out examples table, and a closing quote, with links into the canonical articles. Content is authored in the component (deliberately not `aboutPage`-style DB content — structured, designed layout; `/about` stays biographical). Nav links added in desktop header, mobile menu, and footer between "מאמרים" and "אודות". Static SSR head via `DERECH_SEO` in `STATIC_ROUTE_SEO`, sitemap entry (priority 0.8); no ssrData prefetch needed (static content, global SiteLayout queries only). +1 sitemap test (241→242). | `client/src/pages/Derech.tsx`, `client/src/routes/{lazy.ts,public.tsx}`, `client/src/components/SiteLayout.tsx`, `server/seo.ts`, `server/sitemap.ts`(+test) |
| 2026-08-01 | **Article bodies moved to `mediumtext` (16MB)**: `articles.body`/`articles.excerpt`/`guestPosts.body` were MySQL `text` (64KB), so saving a long RTE article (~70KB HTML) failed with "Failed query: insert into articles". Schema now uses drizzle's `mediumtext`; migration `0015_mediumtext_article_body.sql` (+ idempotent `0015-mediumtext-prod.sql` for Railway) applied to the dev DB via docker exec. `comments.body` intentionally stays `text` (comments are short); `attachments` has no text column. New integration test asserts the column types and round-trips a 100KB article body through the real MySQL. | `drizzle/schema.ts`, `drizzle/0015_mediumtext_article_body.sql`, `drizzle/0015-mediumtext-prod.sql`, `server/article-body-capacity.test.ts` |
| 2026-07-31 | **Collapsible quote block in the editor** (`feature-prompts/collapsible-quote/`): new TipTap node `collapsibleQuote` (`content: "block+"`, `defining`) wraps long quotations and serializes to a native `<details data-collapsible-quote><summary>…</summary><div data-collapsible-quote-body>…</div></details>` — the reader gets a folded box with **no JavaScript** (article bodies go through `dangerouslySetInnerHTML`; the DOCX export flattens tags, so the quote appears in full there). `parseHTML` points `contentElement` at the body wrapper (falling back to a summary-stripped clone) so a `<details>` pasted from elsewhere is adopted without duplicating its title into the content. In the editor the node view is deliberately **always open** — a title `<input>` above the box, per-keystroke `setNodeAttribute`, Enter/↓ drops into the quote. Toolbar button `ChevronsDownUp` toggles the wrap (`toggleWrap`, so a second click unwraps). Registered for **all** editor instances, unlike `htmlEmbed`: `<details>` is inert markup with no script surface. +10 headless tests under happy-dom. | `client/src/components/editor/collapsibleQuote.ts`(+test), `client/src/components/RichTextEditor.tsx`, `client/src/components/RichTextEditor.css`, `client/src/index.css` |
