...
- **I Ching (טקסט ערוך בלבד):** `ichingHexagramText` (number 1..64 + 3 שדות:
  `trigramExplanation`/`interpretation`/`changingLinesNote`), `ichingTrigramText`
  (`trigramKey` → `description` HTML), `ichingIntro` (singleton: מאמר + תוויות).
  המבנה הקבוע (טריגרמות/הקסגרמות/lookup/מנוע ההטלה) **אינו** ב-DB — הוא ב-`shared/iching/`.
