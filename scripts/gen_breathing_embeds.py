...
.html-embed .b8{
  --b8-minus:#2563eb; --b8-minus-soft:#dbeafe;
  --b8-plus:#dc2626;  --b8-plus-soft:#fee2e2;
  --b8-neutral:#cfcabd;
  --b8-bodyA:#f4f0e6; --b8-bodyB:#ddd6c4; --b8-bodyline:#c6bfab;
  --b8-card:var(--surface-1); --b8-ink:var(--text-primary);
  --b8-muted:var(--text-secondary); --b8-line:var(--border-subtle);
...
.html-embed .b8 .b8-rule{background:var(--surface-2);border:1px solid var(--b8-line);
...
/* --- the figure --- */
.html-embed .b8-fig{background:var(--surface-1);border:1px solid var(--b8-line);
...
.html-embed .b8-phase > span{grid-area:1/1;font-size:.85em;font-weight:700;
  ...
  background:var(--surface-2);color:var(--b8-muted);opacity:0}
...
PHASE = ('<div class="b8-phase"><span class="b8-ph-in">שאיפה</span>'
         '<span class="b8-ph-turn">מפנה · כמעט עצירה</span>'
...
<h2>שיטת הנשימה של ה־8</h2>

<p class="b8-lead">לפני כעשרים שנה חיפשתי דרך פשוטה להסביר לעצמי מה בעצם קורה בגוף בזמן
נשימה — לא מבחינה פיזיולוגית, אלא מבחינת <b>הצורה</b>. מה שיצא מזה הוא מפה אחת קטנה,
בצורת הספרה 8, שממנה נגזר הכול.</p>

<h3>בולט ושקוע</h3>
<p>הצעד הראשון הוא לוותר על שמות האיברים ולהסתכל רק על הצורה שלהם: יש איברים
<b>בולטים</b>, שיוצאים החוצה מקו הגוף, ויש איברים <b>שקועים</b>, שנכנסים פנימה.
לבולט נסמן <b>+</b>, לשקוע נסמן <b>−</b>.</p>

<p>אצל הזכר: החזה שקוע (−) והגב בולט (+); איבר המין בולט (+) והישבן שקוע (−).
אצל הנקבה זה הפוך בכל ארבעת האיברים: החזה בולט (+) והגב שקוע (−); אזור המין שקוע (−)
והישבן בולט (+). לא איבר אחד נשאר באותו קוטב — <b>שתי המפות הן היפוך מלא זו של זו</b>.</p>
...
<h3>איך מתרגלים</h3>
<ul>
<li>עמדו או שבו זקוף, והעבירו את תשומת הלב לארבע הנקודות: חזה, גב, איבר המין, ישבן.</li>
<li>סמנו לעצמכם איזה מהן בולטת ואיזו שקועה — לפי המפה שלכם.</li>
<li>שאפו לאורך קו המינוס, נשפו לאורך קו הפלוס.</li>
<li>אל תמהרו במפנה. הוא לא הפסקה בין שתי נשימות — הוא חלק מהמחזור.</li>
</ul>
<p>מחזור מלא בציור שלמטה אורך כ־23 שניות: כשש וחצי שניות שאיפה, חמש שניות מפנה,
כשש וחצי נשיפה, וחמש שניות מפנה. אפשר פשוט לעקוב אחרי הנקודה הנעה ולנשום איתה.</p>

<div class="b8-fig">
%(phase)s
%(svg)s
<div class="b8-cap">שתי המפות זו מול זו — חזה מול חזה, מין מול מין. שימו לב שכל נקודה
פוגשת את הקוטב ההפוך שלה.</div>
</div>
%(legend)s
</div>"""

ART2 = """<div class="b8 b8-2">
<h2>הגוף המאוחד — מה קורה לנשימה ביחסי מין</h2>
...
ולראות מה קורה כששני גופים נפגשים.</p>

<h3>הגופים משלימים זה את זה בכל נקודה</h3>
<p>כשגבר ואישה עומדים זה מול זה, כל מגע ביניהם הוא בין קטבים הפוכים. החזה השקוע שלו פוגש
את החזה הבולט שלה. איבר המין הבולט שלו פוגש את אזור המין השקוע שלה. וגם מה שנשאר בחוץ
משלים: הגב הבולט שלו מול הגב השקוע שלה, הישבן השקוע שלו מול הישבן הבולט שלה.</p>
<p>אין אף נקודה שבה נפגשים שני קטבים זהים. <b>שתי המפות בנויות להיכנס זו בזו</b> — כמו שני
חלקי פאזל שהקו ביניהם הוא אותו קו בדיוק, קמור בצד אחד וקעור בשני.</p>

<h3>מה שנפגש — מתבטל</h3>
<p>ברגע המגע קורה דבר פשוט: שני קטבי החזית — חזה מול חזה, ומין מול מין — הופכים
ל<b>פנימיים</b>. פלוס ומינוס שנפגשים מתבטלים זה מול זה, והם כבר לא חלק מהמעטפת החיצונית
של מה שנוצר. נשארים ארבעה קטבים בלבד, וכולם על הצד החיצוני:</p>
<ul>
<li>גב הזכר — <b>+</b></li>
<li>ישבן הזכר — <b>−</b></li>
<li>גב הנקבה — <b>−</b></li>
<li>ישבן הנקבה — <b>+</b></li>
</ul>

<h3>וזו בדיוק אותה מפה</h3>
<p>עכשיו נסתכל על ארבעת הקטבים האלה כאילו הם גוף אחד. למעלה מצד אחד פלוס, למטה מאותו צד
מינוס; למעלה מהצד השני מינוס, למטה מהצד השני פלוס. אלכסון אחד מחבר שני מינוסים, האלכסון
השני מחבר שני פלוסים, והם נחתכים במרכז — בדיוק בנקודת המגע.</p>

<div class="b8-rule">
<p>זו אותה מפה בדיוק של גוף יחיד, רק בסקאלה אחת למעלה. <b>שני הגופים אינם שני 8 שמסתדרים
זה ליד זה — הם 8 אחת.</b></p>
<p>ומכאן נובעת המסקנה המעשית: בגוף המאוחד יש רק <b>קו מינוס אחד</b> ורק <b>קו פלוס אחד</b>,
ושניהם משותפים. לכן שני בני הזוג <b>שואפים יחד ונושפים יחד</b> — לא כל אחד במחזור שלו,
אלא כגוף אחד שנושם.</p>
</div>

<h3>המפנה המשותף</h3>
<p>גם כאן יש שתי קשתות מפנה, למעלה ולמטה, וגם כאן התנועה כמעט נעצרת בהן. אלא שעכשיו זו
עצירה של שניים. הרגע שבין השאיפה לנשיפה הוא הרגע שבו הכי קל להרגיש שאין שתי נשימות — יש
אחת.</p>

<h3>איך מתרגלים</h3>
<ul>
<li>מצאו קודם, כל אחד לעצמו, את המפה האישית — כמו במאמר הראשון.</li>
<li>במגע, הפנו את תשומת הלב לארבע הנקודות שנשארו בחוץ: שני הגבים ושני הישבנים.</li>
<li>שאפו יחד לאורך קו המינוס, נשפו יחד לאורך קו הפלוס.</li>
<li>אל תמהרו במפנה — הוא המקום שבו האיחוד מורגש הכי חזק.</li>
</ul>

<div class="b8-fig">
%(phase)s
%(svg)s
<div class="b8-cap">שני הפרופילים נכנסים זה בזה. במסגרות המקווקוות: שני המגעים שבהם
+ ו־− מתבטלים והופכים לפנימיים. מה שנשאר — ארבעת הקטבים החיצוניים — מרכיב 8 אחת.</div>
</div>
%(legend)s
</div>"""
